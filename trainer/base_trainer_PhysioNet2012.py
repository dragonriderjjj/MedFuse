...
class BaseTrainer():
    ...
    def train(self, epoch: int = 100):
        ...
        for ep_idx in range(1, epoch+1):
            # Check if we should unfreeze pretrained weights at this epoch
            if hasattr(self.model, 'should_unfreeze_at_epoch') and self.model.should_unfreeze_at_epoch(ep_idx - 1):  # ep_idx is 1-indexed, but should_unfreeze_at_epoch expects 0-indexed
                self.model.unfreeze_pretrained_weights()
                self.logger.info(f"Unfrozen pretrained weights at epoch {ep_idx}")
            ...
            if best_auprc < record['val_AUPRC'][-1] and ep_idx > 25:
                best_auprc = record['val_AUPRC'][-1]
                torch.save(self.model.state_dict(), os.path.join(self.ckp_save_path, f'ckt_ep_{ep_idx}.pth'))
